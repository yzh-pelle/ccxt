...
class bydfi extends bydfi$1["default"] {
    describe() {
        return this.deepExtend(super.describe(), {
            ...
            'exceptions': {
                ...
                'broad': {
                    'is missing': errors.ArgumentsRequired, // {"code":600,"message":"The parameter 'startTime' is missing"}
                },
            },
            'commonCurrencies': {},
            'options': {
    ...
    parseMarket(market) {
        ...
        const maxAmountString = Precise["default"].stringMax(limitMaxQty, marketMaxQty);
        const marketMinQty = this.safeString(market, 'marketMinQty');
        const limitMinQty = this.safeString(market, 'limitMinQty');
        const minAmountString = Precise["default"].stringMin(marketMinQty, limitMinQty);
        ...
        const amountPrecision = Precise["default"].stringDiv(rawAmountPrecision, contractSize);
    ...
    async createOrders(orders, params = {}) {
        ...
        if (length > 5) {
            throw new errors.BadRequest(this.id + ' createOrders() accepts a maximum of 5 orders');
        }
    ...
    async editOrders(orders, params = {}) {
        ...
        const length = orders.length;
        if (length > 5) {
            throw new errors.BadRequest(this.id + ' editOrders() accepts a maximum of 5 orders');
        }
        const ordersRequests = [];
        for (let i = 0; i < orders.length; i++) {
            const rawOrder = orders[i];
    ...
    parsePosition(position, market = undefined) {
        ...
        if (!isFetchPositionsHistory) {
            ...
            contracts = Precise["default"].stringDiv(contracts, contractSize);
        }
    ...
    async setPositionMode(hedged, symbol = undefined, params = {}) {
        if (symbol !== undefined) {
            throw new errors.NotSupported(this.id + ' setPositionMode() does not support a symbol argument. The position mode is set identically for all markets with same settle currency');
        }
        await this.loadMarkets();
